...
const Dashboard = ({ usn }) => {
    ...
    const handleUpdateStatus = async (task_id, status) => {
        try {
            await axios.put(`http://localhost:5000/tasks/${task_id}`, { status });
            const response = await axios.get(`http://localhost:5000/tasks/${usn}`);
            setTasks(response.data.tasks);
        } catch (error) {
                ...
                <ul>
                    {tasks.map((task) => (
                        <li key={task.task_id} className="task-item">
                            ...
                            <button
                                className={`task-button ${task.status === 'Yet to Be Completed' ? 'active' : ''}`}
                                onClick={() => handleUpdateStatus(task.task_id, 'Yet to Be Completed')}
                            >
                                Yet to Be Completed
                            </button>
                            <button
                                className={`task-button ${task.status === 'Completed' ? 'active' : ''}`}
